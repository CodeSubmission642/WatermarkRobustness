...
class Generator_Base:
    ...
    """ Setup for a GAN that generates conferred adversarial samples for a host network
        Note that the GAN only learnsto create the delta perturbation that is added on top of the image
    """
    ...
    def __init__(self,
                 ...
                 weight_path=None):
        """
        @:param host_model The model to extract the fingerprint from
        @:param stolen_models Stolen discriminators from the oracle model
        @:param reference_models Random other discriminators for the same tasks
        ...
        for e in range(epochs):
            for i in range(len(x_train) // batch_size):
                ...
                for idx in true_match_idx:
                    rand_des_classes[idx] = (true_classes[idx] + np.random.randint(1, 9)) % self.nb_class
    ...
    def gan_loss_func_maximum_confidence(self,
        ...
        """
        pred_oracle = self.cl_host_model(x_pred)
        ...
        # Fool the stolen discriminators in the exact same way!
        gamma = 100
        loss_stolen = 0
        ...
        # Reference discriminators should project to correct class
        omega = 100
        loss_random = 0
        for pred_random in preds_random:
            ...
            loss_random += tf.keras.backend.categorical_crossentropy(y_highest, pred_random)
        ...
        eps = self.eps
        delta = x_true - tf.reshape(x_pred, tf.shape(x_true))
        # delta_errors = tf.nn.relu(delta - eps)
        delta_errors = tf.nn.relu(tf.reduce_sum(tf.norm(delta, axis=(1, 2)), axis=-1) - eps)
        # loss_img = alpha * tf.norm(tf.reduce_sum(delta_errors, axis=-1), axis=(1, 2))
        loss_img = alpha * delta_errors
        if self.use_ssim:
            loss_img += alpha * (1-(1+tf.image.ssim(tf.convert_to_tensor(x_pred, dtype=tf.float32), tf.convert_to_tensor(x_true, dtype=tf.float32), 1))/2)
            loss_img /= 2
    ...
    def log_errors(self,
        ...
        # Predict with all discriminators
        pred_val_nn = np.argmax(val_nn.predict(x_fake, verbose=0), 1)                   # Validation model
        pred_orig = np.argmax(self.host_model.predict(x_fake, verbose=0), 1)          # Oracle model
        ...
        total_loss, loss_img, loss_user, loss_stolen, loss_random = self.gan_loss_func_maximum_confidence(x_true,
                                                                                                          x_fake,
                                                                                                          y_true,
                                                                                                          y_des)
        total_loss, loss_img, loss_user, loss_stolen, loss_random = K.eval(tf.reduce_mean(total_loss)), \
                                                                    K.eval(loss_img), K.eval(loss_user),\
                                                                    K.eval(loss_stolen), K.eval(loss_random)
    ...
    def plot_generator_samples(self, x_batch, y_batch, y_des):
        ...
        y_pred_fake = np.argmax(self.host_model.predict(x_fake), axis=1)
